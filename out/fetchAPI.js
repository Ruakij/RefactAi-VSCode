...
exports.look_for_common_errors = exports.fetch_chat_promise = exports.fetch_api_promise = exports.inference_context = exports.non_verifying_ctx = exports.inference_url = exports.save_url_from_login = exports.global_inference_url_from_login = exports.cancel_all_requests_and_wait_until_finished = exports.anything_still_working = exports.wait_until_all_requests_finished = exports.PendingRequest = void 0;
...
class PendingRequest {
    ...
    supply_stream(h2stream, api_fields) {
        this.streaming_error = false;
...
function inference_url(addthis, third_party) {
    ...
    if (!manual_infurl) {
        ...
        if (typeof url_ !== 'string' || url_ === '' || !url_) {
            url = exports.global_inference_url_from_login;
        }
        else {
            url = `${url_}`;
        }
...
exports.inference_url = inference_url;
exports.non_verifying_ctx = fetchH2.context({
    session: {
...
function inference_context(third_party) {
    ...
    let dont_check_certificate = !third_party && !!modified_url;
    if (dont_check_certificate) {
        return exports.non_verifying_ctx;
    }
    else {
        return {
...
function fetch_api_promise(cancelToken, scope, sources, intent, functionName, cursorFile, cursor0, cursor1, maxTokens, maxEdits, stop_tokens, stream, suggest_longthink_model = "", third_party = false) {
    ...
    let model_longthink = vscode.workspace.getConfiguration().get('refactai.longthinkModel') || suggest_longthink_model;
...
exports.fetch_api_promise = fetch_api_promise;
function fetch_chat_promise(cancelToken, scope, messages, function_name, model, stop_tokens, third_party = false) {
...
function look_for_common_errors(json, api_fields) {
    if (json === undefined) {
