...
exports.on_esc_pressed = exports.on_text_edited = exports.on_cursor_moved = exports.inline_rejected = exports.inline_accepted = exports.MyInlineCompletionProvider = void 0;
...
class CacheEntry {
    constructor(completion, created_ts, serial_number) {
...
class MyInlineCompletionProvider {
    constructor() {
        this.cache = new Map(); // LRUCache?
        this.slowdown_rapidfire_ts = 0;
        this.slowdown_rapidfire_count = 0;
        this.slowdown_sustained_ts = 0;
    ...
    async provideInlineCompletionItems(document, position, context, cancelToken) {
        ...
        let cursors_cleaned_cr, cursors_transmit;
        ...
        if (state) {
            ...
            [completion, this_completion_serial_number] = await this.cached_request(state, cancelToken, delay_if_not_cached, file_name, whole_doc, cursor_cr, cursor_transmit, multiline, third_party);
            if (third_party) {
        ...
        let completionItem = new vscode.InlineCompletionItem(completion, 
    ...
    async cached_request(state, cancelToken, delay_if_not_cached, file_name, whole_doc, cursor_cr, cursor_transmit, multiline, third_party) {
        ...
        if (!fail) {
            ...
            if (third_party) {
                ...
                max_tokens = 200;
                [promise, api_fields] = fetch.fetch_api_promise(cancelToken, "completion:" + func, // scope
                sources, "Infill", // intent
                func, file_name, cursor_transmit, cursor_transmit, max_tokens, max_edits, stop_tokens, stream, use_model, third_party);
            }
            else {
                [promise, api_fields] = fetch.fetch_api_promise(cancelToken, "completion", // scope
                sources, "Infill", // intent
                "infill", // scratchpad function
                file_name, cursor_transmit, cursor_transmit, max_tokens, max_edits, stop_tokens, stream, "", third_party);
            }
...
function _extract_extension(feed) {
...
function inline_accepted(serial_number) {
    let feed = _completion_data_feedback_candidate;
